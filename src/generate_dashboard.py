...
function initCharts(viewId) {{
  ...
  if (viewId === 'view-competitor-deep') {{ renderDeepTab(); return; }}
...
function renderDeepOverview() {{
  ...
  var cards = comps.map(function(c, i) {{
    ...
    return '<div class="rounded-xl border shadow-sm p-5 flex flex-col gap-3" style="background:var(--card);border-color:var(--card-border)">'
      ...
      + '<button onclick="setDeepTab(\'detail\');setTimeout(function(){{selectDeepComp(\'' + cid + '\');}},60)" class="text-xs font-semibold text-left mt-auto" style="color:#6366f1">ดูรายละเอียด →</button>'
    + '</div>';
  }});
